...
class AdBillingController extends Controller
{
    ...
    public function adbillingRead_search(Request $request)
    {
        ...
        return view('admission.reports.billing_search');
    }

    public function getadbillingRead_search(Request $request)
    {

        $year = $request->query('year');

        $data = Applicant::leftJoin('ad_examinee_result', 'ad_applicant_admission.id', '=', 'ad_examinee_result.app_id')
